...
@Named
public class TimerStopSpeechCommandProcessor extends AsbtractSpeechCommandProcessor {
	private static final Logger LOGGER = AbstractLoggerFactory.getLogger(TimerStopSpeechCommandProcessor.class);

	private final SoundPlayerService soundPlayerService;

	private final EventBusService eventBusService;

	public TimerStopSpeechCommandProcessor(SoundPlayerService soundPlayerService, EventBusService eventBusService) {
		super("TIMER", "STOP");
	...
	public void process(DistributedEvent event, Command command) {
		...
		this.eventBusService.externalEmit(new DisplayEventData("timer", TimerDisplayData.withHideStatus()));
	}
}
