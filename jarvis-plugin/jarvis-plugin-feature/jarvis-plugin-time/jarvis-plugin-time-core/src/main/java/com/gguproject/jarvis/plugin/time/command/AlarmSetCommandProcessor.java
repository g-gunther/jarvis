...
public class AlarmSetCommandProcessor extends AbstractCommandProcessor{
	private static final Logger LOGGER = AbstractLoggerFactory.getLogger(AlarmSetCommandProcessor.class);

	private final SpeechEventListener speechEventListener;

	public AlarmSetCommandProcessor(SpeechEventListener speechEventListener) {
		super("set", "Set an alarm");
	...
	public CommandOutput process(CommandRequest command) {
		...
		Command alarmCommand = Command.builder("ALARM", "SET").time(timeBuilder).build();
		
		// need to process it as a speech command in order to handle delayed commands
		speechEventListener.processCommand(null, alarmCommand);
